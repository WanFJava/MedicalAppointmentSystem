...
import { CheckCircle, Stethoscope, User, Clock, Activity } from 'lucide-react';
                ...
                if (preselectDoctorId) {
                    ...
                    if (doc) {
                        ...
                        if (initialSelectedDate && initialSelectedSchedule) {
                            ...
                            const scheduleData = await getAvailableSchedules(doc.id);
                            if (isCancelled) return;
                            const grouped = scheduleData.reduce((accumulator, schedule) => {
                                if (!accumulator[schedule.date]) accumulator[schedule.date] = [];
                    ...
                    const docsFiltered = docs.filter(
                        doctor => doctor.specialtyId === parseInt(location.state.selectedSpec)
                    );
                    setFilteredDoctors(docsFiltered);
                }
            } catch (requestError) {
                console.error("Failed to load specialties and doctors", requestError);
            }
        };
        fetchInitialData();
        return () => {
