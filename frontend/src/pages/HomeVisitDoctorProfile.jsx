...
const HomeVisitDoctorProfile = () => {
    ...
    if (loading) {
        return <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem', color: '#6b7280' }}>Đang tải thông tin bác sĩ...</div>;
    }

    if (!doctor) {
        return (
            <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                <h2 style={{ fontSize: '1.5rem', color: '#ef4444', marginBottom: '1rem' }}>Không tìm thấy bác sĩ</h2>
                    ...
                    <div style={{ padding: '2rem', flex: '1 1 45%', display: 'flex', gap: '1.5rem', borderRight: '1px solid #e2e8f0', alignItems: 'flex-start' }}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
                                ...
                                <span style={{ color: '#92400e', fontWeight: '600', fontSize: '1rem' }}>{doctor.averageRating?.toFixed(1) || '0.0'} Đánh giá</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{ padding: '2rem', flex: '1 1 55%', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
                                {Object.keys(schedulesByDate).length === 0 ? (
                                    ...
                                    schedulesByDate[selectedDate]
                                        .filter(s => s.scheduleType === scheduleTypeTab)
                                        .length === 0 ? (
                                            <div style={{ color: '#6b7280', fontSize: '0.95rem' }}>Không có lịch {scheduleTypeTab === 'CLINIC' ? 'tại phòng khám' : 'khám tại nhà'} vào ngày này.</div>
                                        ) : (
                                        schedulesByDate[selectedDate]
                                            .filter(s => s.scheduleType === scheduleTypeTab)
                                            .map(sched => (
                                            <button key={sched.id}
                                                ...
                                                style={{
                                                    padding: '0.5rem 1rem',
                                                    backgroundColor: selectedSchedule?.id === sched.id ? (scheduleTypeTab === 'CLINIC' ? '#0ea5e9' : '#4f46e5') : '#f1f5f9',
                                                    border: 'none',
                        ...
                        <button
                            style={{
                                ...
                                backgroundColor: !selectedSchedule ? '#cbd5e1' : (scheduleTypeTab === 'CLINIC' ? '#0ea5e9' : '#4f46e5'),
                ...
                {doctor.biography && (
                    ...
                    <div style={{ marginTop: '3rem', backgroundColor: 'white', padding: '2.5rem', borderRadius: '0.75rem', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
                        <h3 style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#1f2937', marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            ...
                            Phản hồi gần đây của Bệnh nhân ({feedbacks.length})
                        </h3>

                        {feedbacks.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: '2rem', color: '#6b7280' }}>
                                Bác sĩ này chưa có phản hồi nào.
                            </div>
                        ) : (
