    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Only feature doctors with verified, high patient ratings.
                const highRatedDocs = docs
                    .filter((doctor) => doctor.totalReviews > 0 && Number(doctor.averageRating) >= 4)
                    .sort((a, b) => Number(b.averageRating) - Number(a.averageRating))
                    .slice(0, 6);
                
                setDoctors(highRatedDocs);
                setSpecialties(specs.slice(0, 6));
            } catch (error) {
